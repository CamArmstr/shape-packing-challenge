"""
sa_v3.py — Numba-JIT SA with GJK exact distance.
Drop-in replacement for sa_v2.py with phi_pair_nb replaced by GJK signed distance.
Thompson Sampling operator selection, swap moves, adaptive step size, and stagnation escape.
...
# ── Core distance function: GJK exact signed distance ────────────────────────

from gjk_numba import semicircle_gjk_signed_dist

@nb.njit(cache=True)
def phi_pair_nb(xi, yi, ti, xj, yj, tj):
    """GJK signed distance: positive=separated, negative=overlapping.
    Drop-in replacement for the old phi-function approximation."""
    return semicircle_gjk_signed_dist(xi, yi, ti, xj, yj, tj)
